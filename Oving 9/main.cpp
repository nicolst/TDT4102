    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            case sf::Event::MouseButtonPressed:
                if (event.mouseButton.button == sf::Mouse::Left && game->gameState() == GameState::PLAYING) {
                    ...
                    if (state != GameState::PLAYING) {
                        cout << "Du " << ((state == GameState::WON) ? "VANT!" : "TAPTE!") << endl;
                        cout << "SPILLET ER OVER! Trykk ESC eller Q for å avslutte, eller MELLOMROM for å starte på nytt" << endl;
                    }
                } else if (event.mouseButton.button == sf::Mouse::Right && game->gameState() == GameState::PLAYING) {
                    int row = event.mouseButton.y / tile_size;
                    int col = event.mouseButton.x / tile_size;

                    game->toggleMarkTile(row, col);
                }
                break;
            }
        }
        ...
        for(int row = 0; row < height; ++row) {
            for(int col = 0; col < width; ++col) {
                ...
                if (game->isTileMarked(row, col) && !game->isTileOpen(row, col) && game->gameState() == PLAYING) {
                ...
                if (game->isTileOpen(row, col) || (game->gameState() != PLAYING && game->isTileMine(row, col))) {
                    sf::Text text;
                    text.setStyle(sf::Text::Bold);
                    text.setCharacterSize(tile_size / 2.0);
